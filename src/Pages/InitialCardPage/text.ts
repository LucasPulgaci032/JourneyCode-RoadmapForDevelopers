...
export const operators: string[] = [
  `Operadores são símbolos que determinam a operação realizada entre valores. Dependendo do tipo do operador, o resultado pode ser um número, um booleano, uma string ou outro valor.`,
  `---`,
...
export const loopings: string[] = [
  `Em programação, looping (ou laço de repetição) é uma estrutura utilizada para executar um bloco de código várias vezes, de forma automática. Ele é essencial para automatizar tarefas repetitivas, percorrer listas, processar dados e construir algoritmos mais eficientes.`,
...
export const conditionalDescription: string[] = [
  `Estruturas de decisão em programação são fundamentais, pois determinam qual caminho o código deve seguir com base em uma condição. Internamente, essas estruturas se apoiam em valores booleanos para realizar as verificações.`,
